...
type AtmosphereVariant = 'hero' | 'cyan' | 'violet' | 'amber' | 'horizon';
    ...
    >
      {variant === 'hero' && (
        <>
          ...
          <div className="animate-drift glow-violet absolute -top-10 right-[-10rem] size-[34rem] rounded-full blur-3xl [animation-delay:-11s]" />
      ...
      {variant === 'violet' && (
        <div className="glow-violet absolute top-1/2 left-0 size-[36rem] -translate-y-1/2 rounded-full blur-3xl" />
      )}
      ...
      {variant === 'horizon' && (
        <>
          ...
          <div className="glow-violet absolute bottom-0 right-[-8rem] size-[28rem] rounded-full blur-3xl" />
        </>
      )}
    </div>
  );
